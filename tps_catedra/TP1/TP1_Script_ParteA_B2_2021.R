...
# (se puede chequear si esta instalada mirando en la solapa "packages" en la ventana gráfica)
...
# Exploramos características / estructura del data.frame
...
descriptiva <- CreateTableOne(
                  vars= c("LongSepalo","AnchoSepalo", "LongPetalo","AnchoPetalo"),
                  strata= "especie",
                  datos,
                  test= F)
...
## Con la función lm (linear model) se indica la implementacion de un modelo 
#  lineal para explicar, en este caso, la longitud del sépalo -que es una 
#  variable cuantitativa continua con potencial distribución normal- según la 
...
ggplot(resumen_modelo, aes(x=especie, y=emmean)) +
  ...
  ylim(0,8)+
  ggtitle("Comparación de long de sepalo entre especies", "Media ± Error estándar") +
...
## Para ello se eligieron 9 valores de cobertura del suelo que variaron entre 50 y 90 %, dentro de las cuales se seleccionaron al azar 8 lotes sembrados en el oeste de la Provincia de Buenos Aires a los que posteriormente se les registró el rendimiento.

## En este problema, tendrá la particularidad de que en lugar de contar con una unica muestra, cada grupo contara con su propia muestra, es decir existe un set de datos distinto por equipo. 
...
## 4. Realice un gráfico de dispersión de Y ~ X
...
## 8. Compare con los resultados de sus compañeros. ¿Como son las estimaciones entre los grupos? ¿Por que son distitnas? ¿Alguna es mejor que otra? 
