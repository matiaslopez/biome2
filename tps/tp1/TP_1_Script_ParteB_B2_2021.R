...
# Los gr?ficos ser?n un insumo a lo largo de toda la cursada.
# Necesarios para, por ejemplo, la descripci?n de una muestra, la interpretaci?n de relaciones entre variables o la presentaci?n de un resultado estad?stico.
# En este script se presentan una serie de gr?ficos a realizarse con el R base y con el paquete ggplot.
# Les proponemos explorarlos y familiarizarse con los comandos b?sicos para lograr distintos tipos de gr?ficos.
...
# ?C?mo hacer un grafico "presentable"?
windows(10,6)
...
title(main="Caracter?sticas flor?sticas de Iris", outer=TRUE, cex.main=2)

plot(Datos$LongPetalo~Datos$especie, col=c("yellow","orange","green"),xaxt="n", xlab="", ylab="Longitud del P?talo (cm)", cex.lab=1.2)
legend ("topleft", bty="n",legend = levels(Datos$especie), col = c("yellow","orange","green"), lty=1, lwd=3, cex = 1.3)

hist(Datos$LongPetalo[1:50], main="", col=rgb(1,1,0,1), xlim = c(1,7), ylim=c(0,20), xlab="Longitud del P?talo (cm)", ylab="Frecuencia", cex.lab=1.2)
box("plot",lty="solid")
par(new=TRUE)
